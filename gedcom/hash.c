/* This file is taken from kazlib 1.20 (see URL
   ...
   Only this initial comment has been added (plus two "unused" variable
   attributes and a protection for the original CVS keywords).
 ...
 *
 * $kkId: hash.c,v 1.36.2.11 2000/11/13 01:36:45 kaz Exp $
 * $kkName: kazlib_1_20 $
 */
...
#ifdef KAZLIB_RCSID
static const char rcsid[] = "$kkId: hash.c,v 1.36.2.11 2000/11/13 01:36:45 kaz Exp $";
#endif
...
static hnode_t *hnode_alloc(void *context __attribute__((unused)))
{
    return malloc(sizeof *hnode_alloc(NULL));
}

static void hnode_free(hnode_t *node, void *context __attribute__((unused)))
{
    free(node);
}
